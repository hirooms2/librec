...
public class RatioDataSplitter extends AbstractDataSplitter {
	...
	/**
	 * Initializes a newly created {@code RatioDataSplitter} object with convertor
	 * and configuration.
	 *
	 * @param dataConvertor
...
//			}
	...
	/**
	 * Split the ratings (by date) into two parts: (ratio) training, (1-ratio) test
	 * subsets.
	 *
	 * @param ratio
	 ...
	 * {@code ratio} are preserved for each user as training data with the rest as
	 * test.
	 *
	 * @param ratio
	 *            the ratio of training data
		...
		if (ratio > 0 && ratio < 1) {
			...
			for (int u = 0, um = preferenceMatrix.numRows(); u < um; u++) {
				...
				try {
					...
					LOG.error("This error should not happen because k cannot be outside of the range if ratio is "
							+ ratio);
				}
			}
	...
	/**
	 * Split the ratings of each user (by date) into two parts: (ratio) training,
	 * (1-ratio) test subsets
	 *
	 * @param ratio
	...
	/**
	 * Split the ratings of each item (by date) into two parts: (ratio) training,
	 * (1-ratio) test subsets.
	 *
	 * @param ratio
	 *            the ratio of training data
