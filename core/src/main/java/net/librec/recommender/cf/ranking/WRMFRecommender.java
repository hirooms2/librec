...
public class WRMFRecommender extends MatrixFactorizationRecommender {
	...
	protected void setup() throws LibrecException {
		...
		for (MatrixEntry matrixEntry : trainMatrix) {
			...
			// confindenceMinusIdentityMatrix.set(userIdx, itemIdx, weightCoefficient *
			// matrixEntry.get());
			confindenceMinusIdentityMatrix.set(userIdx, itemIdx,
					Math.log(1.0 + Math.pow(10, weightCoefficient) * matrixEntry.get())); // maybe better for poi
																							// recommender
			preferenceMatrix.set(userIdx, itemIdx, 1.0d);
		}

		try {
			PrintWriter pw = new PrintWriter("train_wrmf.txt");
	...
	protected void trainModel() throws LibrecException {
		...
		for (int iter = 1; iter <= numIterations; iter++) {
			...
			for (int userIdx = 0; userIdx < numUsers; userIdx++) {
				...
				for (int itemIdx : itemList) {
					for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
						YtCuI.set(factorIdx, itemIdx,
								Y.get(itemIdx, factorIdx) * confindenceMinusIdentityMatrix.get(userIdx, itemIdx));
					}
				}
				...
				for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
					for (int itemIdx : itemList) {
						YtCuPu.add(factorIdx, preferenceMatrix.get(userIdx, itemIdx)
								* (YtCuI.get(factorIdx, itemIdx) + Yt.get(factorIdx, itemIdx)));
					}
				}
			...
			for (int itemIdx = 0; itemIdx < numItems; itemIdx++) {
				...
				for (int userIdx : userList) {
					for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
						XtCiI.set(factorIdx, userIdx,
								X.get(userIdx, factorIdx) * confindenceMinusIdentityMatrix.get(userIdx, itemIdx));
					}
				}
				...
				for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
					for (int userIdx : userList) {
						XtCiPu.add(factorIdx, preferenceMatrix.get(userIdx, itemIdx)
								* (XtCiI.get(factorIdx, userIdx) + Xt.get(factorIdx, userIdx)));
					}
				}
		...
		// processing
		try {
			PrintWriter pw = new PrintWriter("C:\\Users\\KIMTAEHO\\MYLIBREC\\data\\interesting/interesting.txt");
			DenseMatrix res = X.mult(Y.transpose());
			
			System.out.println("[" + numUsers + "][" + numItems + "]");
			for (int ui = 0; ui < numUsers; ui++) {
				for (int ii = 0; ii < numItems; ii++) {
					pw.printf("%d\t%d\t%f\n", ui + 1, ii + 1, res.get(ui, ii));
				}
			}
			pw.close();
