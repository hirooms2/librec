			...
			for (int sample = 0, smax = numUsers * 100; sample < smax; sample++) {
				...
				for (int groupUserIdx : groupSet) {
					...
					for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
						...
						double deltaGroup = rho * averageWeight * posItemFactorValue
								+ (1 - rho) * delta * posItemFactorValue - delta * negItemFactorValue;
						tempUserFactors.add(groupUserIdx, factorIdx,
								learnRate * (deriValue * deltaGroup - regUser * groupUserFactorValue));
				...
				for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
					...
					tempItemFactors.add(posItemIdx, factorIdx,
							learnRate * (deriValue * posDelta - regItem * posItemFactorValue));

					double negDelta = -userFactorValue;
					tempItemFactors.add(negItemIdx, factorIdx,
							learnRate * (deriValue * negDelta - regItem * negItemFactorValue));
				}
			}
