...
public abstract class AbstractRecommender implements Recommender {
	 ...
	 * prediction is not bounded. It is useful for building models with no need to
	 * bound predictions.
	 *
	 * @param userIdx
	 *            user index
	...
	/**
	 * predict a specific rating for user userIdx on item itemIdx. It is useful for
	...
	protected boolean isConverged(int iter) throws LibrecException {
		...
		if (Double.isNaN(loss) || Double.isInfinite(loss)) {
			// LOG.error("Loss = NaN or Infinity: current settings does not fit the
			// recommender! Change the settings and try again!");
